...
# Last edited 2025-01-11 to fixe sexual_minority_base
...
packages_essential <- c('stringr','openxlsx','readr','dplyr')
...
output_file_path <- 
  ...
  paste0(file_name_root," (selected columns) - ", Breakdown_included,".xlsx")
#}else{
...
for (sheet_name in sheet_names) {
  ...
  if (sheet_name == "YOUR JOB Q1-3") {
    ...
    selected_columns <- cbind(selected_columns, Q1_column, Q3g_column)
    ...
    selected_columns <- cbind(gender_columns, age_columns, ethnicity_columns, sexuality_columns)
    ...
    ### Religion Not needed as already aggregated by rows in the script
    selected_columns <- cbind(gender_columns, age_columns, ethnicity_columns)
...
#*** combined_data initiated -----
#*** If org type files -----
if (grouping == "organisation type results"){
  combined_data <- combined_data[!combined_data$`Benchmarking_group` %in% 
...
if (grouping == "organisational results"){
  if ("Benchmarking_group" %in% colnames(combined_data)) {
    combined_data <- combined_data[!combined_data$`Benchmarking_group` %in% 
                                     c('Community Surgical Services',
                                       'CSUs',
                                       'Social Enterprises - Community', 
...
combined_data[, grep("^Q", colnames(combined_data))] <- 
  lapply(combined_data[, grep("^Q", colnames(combined_data))], function(x) {
...
if ("Q12b % Often" %in% colnames(combined_data) && "Q12b % Always" %in% colnames(combined_data)) {
  ...
  combined_data$Burned_out_sometimes_rate <- combined_data$Burned_out_rate  + combined_data$`Q12b % Sometimes`
...
if ("Q24c % 41-50" %in% colnames(combined_data)) {
  ...
  colnames(combined_data)[colnames(combined_data) == 'Q24c Base (number of responses)'] <- 'Older_base'
} else if ("Q26c % 41-50" %in% colnames(combined_data)) {
  combined_data$Older_rate <- combined_data$`Q26c % 51-65` + combined_data$`Q26c % 66+`
  colnames(combined_data)[colnames(combined_data) == 'Q26c Base (number of responses)'] <- 'Older_base'
} else if ("Q27c % 41-50" %in% colnames(combined_data)) {
  combined_data$Older_rate <- combined_data$`Q27c % 51-65` + combined_data$`Q27c % 66+`
  colnames(combined_data)[colnames(combined_data) == 'Q27c Base (number of responses)'] <- 'Older_base'
} else {
...
if ("Q24a % Female" %in% colnames(combined_data)) {
  ...
  colnames(combined_data)[colnames(combined_data) == 'Q24a Base (number of responses)'] <- 'Female_base'
}else if ("Q26a % Female" %in% colnames(combined_data)){
  combined_data$Female_rate <- combined_data$`Q26a % Female`
  colnames(combined_data)[colnames(combined_data) == 'Q26a Base (number of responses)'] <- 'Female_base'
}else if ("Q27a % Female" %in% colnames(combined_data)){
  combined_data$Female_rate <- combined_data$`Q27a % Female`
  colnames(combined_data)[colnames(combined_data) == 'Q27a Base (number of responses)'] <- 'Female_base'
} else {
...
if ("Q26 % Heterosexual or Straight" %in% colnames(combined_data)) {
  combined_data$Sexual_minority_rate <- 100 - combined_data$`Q26 % Heterosexual or Straight`
  colnames(combined_data)[colnames(combined_data) == 'Q26 Base (number of responses)'] <- 'Sexual_minority_base'
}else if ("Q28 % Heterosexual or Straight" %in% colnames(combined_data)){
  browser()
  combined_data$Sexual_minority_rate <- 100 - combined_data$`Q28 % Heterosexual or Straight` # corrected 2025-01-11
  colnames(combined_data)[colnames(combined_data) == 'Q28 Base (number of responses)'] <- 'Sexual_minority_base'
}else if ("Q29 % Heterosexual or Straight" %in% colnames(combined_data)){
  browser()
  combined_data$Sexual_minority_rate <- 100 - combined_data$`Q29 % Heterosexual or Straight` # corrected 2025-01-11
  colnames(combined_data)[colnames(combined_data) == 'Q29 Base (number of responses)'] <- 'Sexual_minority_base'
}else {
...
if ("Q25 % White - Irish" %in% colnames(combined_data)) {
  selected_columns <- combined_data %>% dplyr::select(matches("Q25 % White"))
  combined_data$Ethnic_minority_rate <- 100 - rowSums(selected_columns, na.rm = TRUE)
  colnames(combined_data)[colnames(combined_data) == 'Q25 Base (number of responses)'] <- 'Ethnic_minority_base'
} else if ("Q27 % White - Irish" %in% colnames(combined_data)) {
  selected_columns <- combined_data %>% dplyr::select(matches("Q27 % White"))
  combined_data$Ethnic_minority_rate <- 100 - rowSums(selected_columns, na.rm = TRUE)
  colnames(combined_data)[colnames(combined_data) == 'Q27 Base (number of responses)'] <- 'Ethnic_minority_base'
} else if ("Q28 % White - Irish" %in% colnames(combined_data)) {
  selected_columns <- combined_data %>% dplyr::select(matches("Q28 % White"))
  combined_data$Ethnic_minority_rate <- 100 - rowSums(selected_columns, na.rm = TRUE)
  colnames(combined_data)[colnames(combined_data) == 'Q28 Base (number of responses)'] <- 'Ethnic_minority_base'
} else {
...
if ("Q31 % Medical / Dental - Consultant" %in% colnames(combined_data)) {
  ...
  colnames(combined_data)[colnames(combined_data) == 'Q31 Base (number of responses)'] <- 'Med_Dental_base'
  combined_data$Med_Dental_rate <- combined_data$`Q31 % Medical / Dental - Consultant` + combined_data$`Q31 % Medical / Dental - In Training` + combined_data$`Q31 % Medical / Dental - Other`
  colnames(combined_data)[colnames(combined_data) == 'Q31 Base (number of responses)'] <- 'Med_Dental_base'
  ...
  colnames(combined_data)[colnames(combined_data) == 'Q33 Base (number of responses)'] <- 'Med_Dental_base'
  ...
  colnames(combined_data)[colnames(combined_data) == 'Q35 Base (number of responses)'] <- 'Med_Dental_base'
} else {
...
if ("Q3g % Agree" %in% colnames(combined_data) && "Q3g % Strongly agree" %in% colnames(combined_data)) {
  combined_data$Time_demands_meet_rate <- combined_data$`Q3g % Agree` + combined_data$`Q3g % Strongly agree`
  colnames(combined_data)[colnames(combined_data) == 'Q3g Base (number of responses)'] <- 'Time_demands_meet_base'
} else {
...
#*** Discrimination -----
if ("Q16a % Yes" %in% colnames(combined_data) && "Q16a % Yes" %in% colnames(combined_data)) {
  combined_data$Discrimination_public_rate <- combined_data$`Q16a % Yes`
  combined_data$Discrimination_internal_rate <- combined_data$`Q16b % Yes`
  colnames(combined_data)[colnames(combined_data) == 'Q16a Base (number of responses)'] <- 'Discrimination_public_base'
  colnames(combined_data)[colnames(combined_data) == 'Q16b Base (number of responses)'] <- 'Discrimination_internal_base'
} else {
