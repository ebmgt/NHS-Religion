...
packages_zehan <- (c("betareg", "readr"))
...
if (grepl("data_trusts_burnout.csv", filename)) {
...
dat$Respondents - dat$stress.numerator 
#* Meta-regression -----
dat$Ethnic_minority_rate
meta_all_years<- rma.mv(yi = stress.rate, V = stressvariance, 
                        mods = ~ Emotional_public_rate + year, random = ~ 1 | religion, 
                        data = dat)
