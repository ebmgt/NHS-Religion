...
#---------------------------------
library(meta) # Builds on metafor: metamean and metaprop
library(metafor) # Orginal meta-analysis package
library(lm.beta) # Creates standardied coefficients from non-meta-analytic regression models
library(grid)
library (ggplot2)
#library(plyr)
#library(Rcmdr)
...
data.temp <- read.csv(filename, header=TRUE, sep=",", na.strings="NA", dec=".", stringsAsFactors=FALSE, strip.white=TRUE)
...
plotheight = 700
meta1 <- metaprop(Burned.Out_Often.Always,Total, studlab = Religion, data=data.temp, byvar = Group, comb.fixed=FALSE,hakn=TRUE, title = "Rates of burnout")
